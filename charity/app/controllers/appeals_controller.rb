class AppealsController < ApplicationController
  ...
  def edit
    ...
    @charitypics=CharityMedia.where(:charityusersquare_id =>@appeal.id,:types=>@appeal.charitytype )
  ...
  def create
    ...
    @appeal.active = true
     ...
     appeal_media.appeal_id = appeal_id
     appeal_media.save
     created_appeal_id=appeal_media.id
     path="#{Rails.root}/public/images/appeal/#{created_appeal_id}.jpg"
     temp.push path
     ...
     @update.save()
      ...
      Delayed::Job.enqueue(MailingJob.new(@appeal.id, "appeal"), -3, @time_enque.minutes.from_now.in_time_zone(@tz))
  ...
  def removepic
   @mediaid=params[:pic]
   CharityMedia.where(:picture=>@mediaid,:types=>'appeal').first.destroy
   File.delete("public/images/appeal/#{params[:pic]}.jpg") if File.exist?("public/images/appeal/#{params[:pic]}.jpg")
   render :js => 'deleted'
  end 
  ...
  private
    ...
    def appeal_params
      params.require(:appeal).permit(:user_id, :charitytype, :charityusersquare_id, :description, :enddate, :image_array, :startdate, :active, :appeal_type, :specific_appeal_description, :target_fund_amount)
    end
end
