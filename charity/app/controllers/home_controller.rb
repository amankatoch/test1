class HomeController < ApplicationController
  ...
  def pay
    if current_user.present?
      if current_user.id == params[:user_id].to_i
        ...
        data = {"returnUrl" => "127.0.0.1:3000/order_confirmation?bet_id=#{params[:bet_id].to_i}&user_id=#{params[:user_id].to_i}&price=#{params[:price]}", "requestEnvelope" => {"errorLanguage" => "en_US"}, "currencyCode"=>"USD", "receiverList"=>{"receiver"=>[{ "email"=>User.find_by_id(params[:user_id].to_i).paypal_email, "amount"=>params[:price].to_f - ((5 * params[:price].to_f)/100), "betid"=>params[:bet_id], "userid"=> params[:user_id]}, {"email"=>"charity-admin@esferasoft.com", "amount"=>(5 * params[:price].to_f)/100}]}, "cancelUrl"=>"127.0.0.1:3000/canceled_payment_request", "actionType"=>"PAY", "ipnNotificationUrl"=>"127.0.0.1:3000/ipn_notification?userid=#{params[:user_id]}&betid=#{params[:bet_id]}&charitytype=#{params[:charitytype]}"}
  ...
  def manage_purchase
    ...
    @bet= PaymentDetail.where(:receiver_email=>@user_email , :charitytype=>"Bets").group('bet_id')
    @appeal = PaymentDetail.where(:receiver_email=>@user_email, :charitytype=>"Appeals").group('appeal_id')
    @transaction= (@bet << @appeal).paginate(:page => params[:page], :per_page => 6)
    else
    @user_email=current_user.paypal_email
    @bet= PaymentDetail.where(:sender_email=>@user_email , :charitytype=>"Bets").group('bet_id')
    @appeal = PaymentDetail.where(:sender_email=>@user_email, :charitytype=>"Appeals").group('appeal_id')
    @transaction= (@bet << @appeal).paginate(:page => params[:page], :per_page => 6)
    end
  end
