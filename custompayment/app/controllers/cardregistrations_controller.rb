class CardregistrationsController < ApplicationController
  before_action :set_cardregistration, only: [:show, :edit, :update, :destroy]
  ...
  respond_to :html
    ...
    def create
    @registration = Cardregistration.new(registration_params)
    render :plain => @some_object.inspect
    return
   ...
   def registration_params
    params.require(:registration).permit(card_attributes: [
                                              :first_name, :last_name, :card_type, :card_number,
                                              :card_verification, :card_expires_on])


     def new
    @registration = Cardregistration.new
